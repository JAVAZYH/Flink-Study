...
object newTopNGroupStatic {
  def main(args: Array[String]): Unit = {
    ...
    //主要key，用来进行keyby操作
    val mainKeyPos = "uin".split('|').map(str => {
      parseColumnsName(str, inColumnLists.head).toInt - 1
    })

    //统计结果时长(秒)
    val timeValid = "9999".toLong
    val sortType = "AESC_SORT"
    val topNumber = "5".toInt
    ...
    class TopNKeyedProcessFunction extends KeyedProcessFunction[String,String,String] {
      ...
      override def processElement(value: String, ctx: KeyedProcessFunction[String, String, String]#Context, out: Collector[String]): Unit = {
        ...
        try {
          ...
          if(isDistinct){
            //判断是否含有key
            val keyArrBuff=ArrayBuffer[Double]()
            for (key <- cacheArrBuff) {
              keyArrBuff.append(key._1)
            }
            val index: Int = keyArrBuff.indexOf(orderField)
            //如果该key不存在加入元素
            if(index==(-1)){
            ...
            //如果存在更新元素
              ...
              cacheArrBuff.update(index, orderField->value)
            }
          }else{
            cacheArrBuff.append(orderField->value)
          }
          ...
          if(sortType=="DESC_SORT"){
            cacheArrBuff=cacheArrBuff.sorted(Ordering.Tuple2(Ordering.Double.reverse,Ordering.String.reverse))
          }else{
            cacheArrBuff=cacheArrBuff.sorted(Ordering.Tuple2(Ordering.Double,Ordering.String.reverse))
          }
